...
const startGlobalGameTimer = () => {
  const totalTime = speed * count * 1000;
  console.log(`Starting timer: ${totalTime}ms - Input will be disabled after expiry`);
  
  // clearAllTimersSafely();
  
  setGameElapsedTime(0);
  setTimeRemaining(totalTime);
  setAvailableQuestions(new Set([0]));
  setIsInputDisabled(false); // Enable input at start
  ...
  const gameTimer = setInterval(() => {
    setGameElapsedTime(prev => {
      ...
      // When time expires - disable input but allow submit
      if (newRemaining <= 0) {
        clearInterval(gameTimer);
        setTotalGameTimer(null);
        setIsInputDisabled(true); // DISABLE INPUT AFTER TIME EXPIRES
        
        // Make all questions available for review/submit
        ...
        console.log('Time expired - input disabled, submit only');
        return newElapsed;
      }
  ...
  <div className="number-pad" style={{ position: 'relative', zIndex: 1000 }}>
    <div className="number-pad-row">
      {[7, 8, 9].map(num => (
        <button 
          key={num} 
          onMouseDown={(e) => {
            if (isInputDisabled) return; // Don't allow input when disabled
            e.preventDefault();
            e.stopPropagation();
            handleNumberPadInput(num.toString(), e);
          }}
    ...
    <div className="number-pad-row">
      ...
      <button 
        ...
        style={{ 
          pointerEvents: 'auto',
          backgroundColor: isInputDisabled ? '#28a745' : '#6c5ce7', // Green when input disabled
          opacity: 1 // Submit button always enabled
        }}
      >
        Submit
      </button>
